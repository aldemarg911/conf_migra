DECLARE
    /******************************************************************************
    NOMBRE:       03_Mig_personas_rel_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de mig_personas_rel - PRODUCCION

    REVISIONES:
    Ver        Fecha        Autor             Descripción
    ---------  ----------  ---------------  ------------------------------------
    1.0        ??/??/????  cxhgome            1. Creación del script.
/* Formatted on 12/09/2017 11:36 (Formatter Plus v4.8.8) - (CSI-Factory Standard Format v.2.0) */
/*
   ...
   --
    CURSOR lc_mig_personas_rel IS
     SELECT 17413 NCARGA, 1 CESTMIG, a.MIG_PK, a.MIG_FK, a.FKREL, a.CTIPREL, a.PPARTICIPACION, a.ISLIDER
      ...
      --select * from mig_personas_rel_conf      
    ORDER BY a.mig_pk
        ;         
        --SELECT * FROM mig_personas_rel s; 
        --SELECT * FROM mig_personas_rel_conf s;
        --SELECT * FROM mig_garanseg s WHERE s =  ORDER BY s ;           
        --SELECT * FROM mig_movrecibo s;
        --SELECT * FROM mig_personas_rel_conf s for update WHERE s.mig_pk = 'RD30094778';
...
SELECT * FROM mig_personas_rel_conf WHERE mig_pk = 'A00005041';
SELECT * FROM cesionesrea s ;
SELECT COUNT(*) FROM mig_personas_rel s WHERE s.ncarga = 17413;
SELECT * FROM mig_personas_rel s WHERE s.ncarga = 17413 AND s.cestmig = 2;
SELECT * FROM mig_personas_rel s WHERE s.ncarga = 17413;
SELECT * FROM mig_personas s WHERE s.mig_pk = '900587410';
/*SELECT * FROM mig_personas_rel s WHERE s.ncarga = 0;
SELECT count(*) FROM mig_personas_rel_conf s WHERE s.ncarga = 17411;
delete FROM mig_personas_rel s WHERE s.ncarga = -17411;*/
--SELECT * FROM control_error s WHERE s.donde = '03_mig_personas_rel' and seqerror > 9537681 order by seqerror;
DELETE FROM control_error s WHERE s.donde = '03_mig_personas_rel'
SELECT * FROM control_error s WHERE s.donde = '03_mig_personas_rel' order by seqerror;
SELECT * FROM mig_logs_axis s WHERE s.ncarga = 17413 AND s.seqlog > 2432273;
SELECT * FROM mig_personas_rel WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_personas_rel);
SELECT * FROM mig_personas_rel s, mig_personas_rel p
