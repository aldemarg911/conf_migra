  ...
  LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        --
        p_control_error('CXHGOME','01_Borra_Personas_Direcciones','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
  ...
  LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_Personas_Per_Rel','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
  ...
  LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_Personas_Per_Contac','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
 ...
 OPEN lc_mig_personas_1; LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_PER_NACIONALID','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
   ...
   EXIT WHEN lc_mig_personas_1%NOTFOUND;   
   --
  END LOOP;   CLOSE lc_mig_personas_1;
  ...
  OPEN lc_mig_personas_1; LOOP
    ...
    BEGIN
      ...
      /*FOR i IN l_per.FIRST..l_per.LAST LOOP
  ...
  END LOOP;*/
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_PER_DETPER','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
 ...
 OPEN lc_mig_personas_1; LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_PER_REGIMENFISCAL','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
 ...
 OPEN lc_mig_personas_1; LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_PER_CCC','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
 ...
 /*OPEN lc_mig_personas_1; LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_AGENTES','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --
   END;
 ...
 OPEN lc_mig_personas_1; LOOP
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        l_idx   := SQL%BULK_EXCEPTIONS(i).error_index;
        --
        p_control_error('CXHGOME','01_Borra_PER_PERSONAS','['||l_idx||']: '||l_errno||' - '||l_msg||'.. PK: '||l_per(i).nnumide);
        --
      END LOOP;
      --      
   END;
...
SELECT * FROM control_error s WHERE trunc(s.fecha) = trunc(sysdate) and id = 'CXHGOME' and donde != '01_Actualiza_Personas_PK';
