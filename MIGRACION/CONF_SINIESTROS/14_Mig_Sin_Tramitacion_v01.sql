DECLARE
    /******************************************************************************
    NOMBRE:       14_Mig_Sin_Tramitacion_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de MIG_SIN_TRAMITACION - PRODUCCION
...
*/
   ...
   --
    CURSOR lc_mig_sin_tramitacion IS
     ...
     --select count(*)
      FROM mig_sin_tramitacion_cl2 a, mig_sin_siniestro s
     WHERE 1 = 1
...
BEGIN
  ...
  LOOP
   ...
   -----------------------------------------
END;
--DELETE FROM mig_sin_tramitacion;
/*SELECT COUNT(*) FROM mig_sin_tramitacion WHERE cestmig = 2;
SELECT * FROM mig_sin_tramitacion_conf s ;
SELECT * FROM mig_sin_tramitacion_conf s WHERE s.crespue IS NULL
...
--SELECT * FROM control_error s WHERE s.donde = '03_mig_sin_tramitacion' and seqerror > 9537681 order by seqerror;
/*DELETE FROM control_error s WHERE s.donde = '14_mig_sin_tramitacion'
SELECT * FROM control_error s WHERE s.donde = '14_mig_sin_tramitacion' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_sin_tramitacion);
SELECT * FROM mig_sin_tramitacion s, mig_personas p
WHERE s.mig_fk = p.mig_pk
  AND p.idperson <> 0;
--
SELECT * FROM mig_sin_tramitacion s WHERE cestmig = 2;
-- 09_Mig_MovSeguro_v01.sql
-- 
SELECT * FROM mig_logs_axis s WHERE s.ncarga = 17452 AND s.seqlog > 0;
*/
--
