DECLARE
    /******************************************************************************
    NOMBRE:       23_Mig_Sin_Tramita_Pago_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de mig_sin_tramita_pago - SINIESTROS

    REVISIONES:
    Ver        Fecha        Autor             Descripción
    ---------  ----------  ---------------  ------------------------------------
    1.0        19/12/2017  aluna            1. Creación del script.
...
SELECT 17445 NCARGA, 1 CESTMIG, a.MIG_PK, a.MIG_FK, a.MIG_FK2, 0 SIDEPAG, 0 NSINIES, a.NTRAMIT, 0 SPERSON, a.CTIPDES, a.CTIPPAG, NVL(a.CCONPAG, 0), a.CCAUIND, 
	NVL(a.CFORPAG, 0), a.FORDPAG, a.CTIPBAN, a.CBANCAR, a.CMONRES, a.ISINRET, a.IRETENC, a.IIVA, a.ISUPLID, a.IFRANQ, a.IRESRCM, a.IRESRED, a.CMONPAG,
	 ...
	 null TOBSERVA, 0 IOTROSGAS, 0 IOTROSGASPAG, 0 IBASEIPOC, 0 IBASEIPOCPAG, 0 IIPOCONSUMO, 0 IIPOCONSUMOPAG
       FROM mig_sin_tramita_pago_cl2 a, mig_sin_tramitacion s, mig_personas p
     WHERE 1 = 1
     AND s.mig_pk = a.mig_fk2
     AND p.mig_pk = a.mig_fk
     AND s.nsinies <> 0
      ORDER BY a.mig_pk;
        --SELECT * FROM mig_sin_tramita_movpago_conf s for update; WHERE s.mig_pk = '131050100DL0200681';
        --SELECT * FROM mig_sin_tramita_movpago s for update WHERE s.mig_pk = 'RD30094778';
...
BEGIN
  ...
  LOOP
   ...
   -----------------------------------------
END;
--DELETE FROM mig_sin_tramita_movpago;
/*SELECT * FROM mig_sin_tramita_movpago;
SELECT * FROM mig_sin_tramita_movpago_conf s ;
SELECT * FROM mig_sin_tramita_movpago_conf s WHERE s.crespue IS NULL
...
--SELECT * FROM control_error s WHERE s.donde = '03_mig_sin_tramita_movpago' and seqerror > 9537681 order by seqerror;
/*DELETE FROM control_error s WHERE s.donde = '03_mig_sin_tramita_movpago'
SELECT * FROM control_error s WHERE s.donde = '45_mig_sin_tramita_movpago' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_sin_tramita_movpago);
SELECT * FROM mig_sin_tramita_movpago s, mig_personas p
