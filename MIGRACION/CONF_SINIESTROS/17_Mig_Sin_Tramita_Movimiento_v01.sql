DECLARE
    /******************************************************************************
    NOMBRE:       17_Mig_sin_tramita_movimiento_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de MIG_SIN_TRAMITA_MOVIMIENTO - PRODUCCION
   ...
   --
    CURSOR lc_mig_sin_tramita_movimiento IS
     SELECT 17443 NCARGA, 1 CESTMIG,  a.MIG_PK, a.MIG_FK, 0 NSINIES, a.NTRAMIT, a.NMOVTRA,
      ...
      --Select count(*)
      FROM mig_sin_tra_movimiento_cl2 a, mig_sin_tramitacion s
     WHERE 1 = 1
      ...
      ORDER BY a.mig_pk
        ;
        --SELECT * FROM mig_sin_tramita_movimi_conf s for update; WHERE s.mig_pk = '131050100DL0200681';
        --SELECT * FROM mig_sin_tramita_movimiento s for update WHERE s.mig_pk = 'RD30094778';
...
BEGIN
  ...
  LOOP
    --
    FETCH lc_mig_sin_tramita_movimiento BULK COLLECT INTO l_reg_mig_mcc LIMIT v_limit_records;
    --
    ...
    WHEN DML_ERRORS THEN
      ...
      LOOP
        ...
        --
        p_control_error('CXHGOME','17_mig_sin_tramita_movimiento','['||l_idx||']: '||l_errno||' - '||l_msg||'. PK: '||l_reg_mig_mcc(l_idx).mig_pk);
        --
      END LOOP;
      --
    END;
   ...
   -----------------------------------------
END;
--DELETE FROM mig_sin_tramita_movimiento;
/*SELECT * FROM mig_sin_tramita_movimiento;
SELECT * FROM mig_sin_tramita_movimi_conf s ;
SELECT * FROM mig_sin_tramita_movimi_conf s WHERE s.crespue IS NULL
...
--SELECT * FROM control_error s WHERE s.donde = '03_mig_sin_tramita_movimiento' and seqerror > 9537681 order by seqerror;
/*DELETE FROM control_error s WHERE s.donde = '03_mig_sin_tramita_movimiento'
SELECT * FROM control_error s WHERE s.donde = '45_mig_sin_tramita_movimiento' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_sin_tramita_movimiento);
SELECT * FROM mig_sin_tramita_movimiento s, mig_personas p
...
-- 
SELECT * FROM TIPOS_INDICADORES s WHERE;
--
SELECT * FROM all_tables WHERE table_name LIKE '%MIG_SIN_TRAMITA%MOV%';
--
s
*/