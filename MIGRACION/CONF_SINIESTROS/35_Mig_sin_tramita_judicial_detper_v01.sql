DECLARE
    /******************************************************************************
    NOMBRE:       35_mig_sin_tram_judi_detper_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de MIG_SIN_TRAM_JUDI_DETPER - PRODUCCION
   ...
   --
    CURSOR lc_mig_sin_tram_judi_detper IS
     SELECT 17459 NCARGA, 1 CESTMIG, 0 nsinies, 0 ntramit, a.MIG_PK,	a.MIG_FK,	a.MIG_FK2,	a.NORDEN,	a.NROL,	a.NPERSONA,	a.NTIPPER,	
      ...
      FROM /*transformacion.*/mig_sin_tram_judi_detper_conf a, mig_sin_siniestro s, mig_sin_tramita_judicial t
     WHERE 1 = 1
      ...
      ORDER BY a.mig_pk
        ...
        --SELECT * FROM mig_sin_tram_judi_detper_conf s for update; WHERE s.mig_pk = '131050100DL0200681';
        --SELECT * FROM mig_sin_tram_judi_detper s for update WHERE s.mig_pk = 'RD30094778';
...
BEGIN
  ...
  LOOP
   ...
   -----------------------------------------
END;
--DELETE FROM mig_sin_tram_judi_detper;
SELECT * FROM mig_sin_tram_judi_detper_conf;
SELECT * FROM mig_sin_tram_judi_detper s ;
SELECT * FROM mig_sin_tram_judi_detper s WHERE s.crespue IS NULL
...
--SELECT * FROM control_error s WHERE s.donde = '03_mig_sin_tram_judi_detper' and seqerror > 9537681 order by seqerror;
DELETE FROM control_error s WHERE s.donde = '03_mig_sin_tram_judi_detper'
SELECT * FROM control_error s WHERE s.donde = '45_mig_sin_tram_judi_detper' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_sin_tram_judi_detper);
SELECT * FROM mig_sin_tram_judi_detper s, mig_personas p
