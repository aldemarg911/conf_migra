DECLARE
   ...
   -- := 'AXIS_CSI';   -- Usuario que quedará en pie
   vsseguro       NUMBER;
...
BEGIN
   ...
   FOR reggar IN c_seguros LOOP
      ...
      p_tab_error(f_sysdate, f_user, 'bucle 1 reggar', 1, ' vsseguro --> ' || vsseguro, 'OK');
...
--            WHERE sseguro = vsseguro;
      ...
      FOR reg IN recibos LOOP
         p_tab_error(f_sysdate, f_user, 'bucle 2 recibos', 1,
   ...
   p_tab_error(f_sysdate, f_user, 'Borrado SEGUROS', 1,
   ...
   p_tab_error(f_sysdate, f_user, 'Borrado ESTPERSONAS', 2,
   ...
   FOR reg IN (SELECT sperson
    ...
    LOOP
      --
      p_tab_error(f_sysdate, f_user, 'bucle 3 regper', 1, ' sperson --> ' || reg.sperson, 'OK');
      --
      DELETE FROM asegurados
            WHERE sperson = reg.sperson;
   ...
   p_tab_error(f_sysdate, f_user, 'Borrado PERSONAS', 3, ' Hemos borrado las PERSONAS -->  OK ! ', 'OK');
   ...
   p_tab_error(f_sysdate, f_user, 'Borrado ESTSEGUROS', 4,
   ...
   p_tab_error(f_sysdate, f_user, 'Borrado datos de REASEGURO', 8,
...
--         WHERE cempres = vcempres;
   ...
   p_tab_error(f_sysdate, f_user, 'Borrado dependencias EMPRESA', 9,
               ' Hemos borrado dependencias EMPRESA -->  OK ! ', 'OK');
EXCEPTION
   WHEN OTHERS THEN
      DBMS_OUTPUT.put_line('Error al borrar ' || SQLERRM);
      p_tab_error(f_sysdate, f_user, 'Error al borrar', 0, SQLCODE, SQLERRM);
      ROLLBACK;
END;
--
--SELECT * FROM tab_error WHERE TRUNC(ferror) = TRUNC(SYSDATE) ORDER BY ferror;
--
