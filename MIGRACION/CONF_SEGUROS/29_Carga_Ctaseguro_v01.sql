-- 32_Mig_Agensegu_v01.sql
DECLARE
   vdummy            NUMBER;
   vncarga           mig_cargas.ncarga%TYPE := 20061;
   v_nerr            NUMBER;
   v_cant            NUMBER;
   v_seqlog          NUMBER := 0;
   --
...
BEGIN
  ...
  SELECT pac_contexto.f_inicializarctx(pac_parametros.f_parempresa_t(24, 'USER_BBDD'))
    ...
    FROM dual;
    --
    -- 0. 27_Mig_Ctaseguro_v01.sql.sql  (Crea registros de carga)
    -- 1. 28_Mig_Ctaseguro_v01.sql.sql  (Migra registros de _CONF a MIG_)
    -- 2. - void -
    -- 3. Ejecutar este script
    -- 4. Actualizar T[s] documento DOC-108
    --
    pac_mig_axis_conf.p_migra_cargas(pid     => 'MIG_CTASEGURO_TXT',
                                     ptipo   => 'C',
                                     pncarga => vncarga);
    --
