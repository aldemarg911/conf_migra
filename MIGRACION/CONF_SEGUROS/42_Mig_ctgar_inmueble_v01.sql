DECLARE
    /******************************************************************************
    NOMBRE:       42_Mig_ctgar_inmueble_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de SITRIESGO - PRODUCCION
...
*/
   ...
   --
    CURSOR lc_mig_ctgar_inmueble IS
     SELECT 17426 NCARGA,1 CESTMIG, a.MIG_PK, a.MIG_FK, a.NNUMESCR, a.FESCRITURA,
           a.TDESCRIPCION, a.TDIRECCION, a.CPAIS, a.CPROVIN, a.CPOBLAC, a.FCERTLIB
       --SELECT COUNT(*)
      FROM mig_ctgar_inmueble_cl1 a, Mig_Ctgar_Det d
     WHERE 1 = 1     
       AND a.mig_fk = d.mig_pk
  ORDER BY a.mig_pk
        --AND s.mig_pk = '131050100DL0200681'
        ; 
        --SELECT * FROM mig_ctgar_inmueble_conf s WHERE s.mig_pk = '131050100DL0200681';
   --
...
BEGIN
  ...
  LOOP
   ...
   -----------------------------------------
END;
--DELETE FROM mig_ctgar_inmueble;
SELECT * FROM mig_ctgar_inmueble;
SELECT * FROM mig_ctgar_inmueble_conf s ;
SELECT * FROM mig_ctgar_inmueble_conf s WHERE s.crespue IS NULL
...
--SELECT * FROM control_error s WHERE s.donde = '03_mig_ctgar_inmueble' and seqerror > 9537681 order by seqerror;
DELETE FROM control_error s WHERE s.donde = '03_mig_ctgar_inmueble'
SELECT * FROM control_error s WHERE s.donde = '37_mig_ctgar_inmueble' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_ctgar_inmueble);
SELECT * FROM mig_ctgar_inmueble s, mig_personas p
