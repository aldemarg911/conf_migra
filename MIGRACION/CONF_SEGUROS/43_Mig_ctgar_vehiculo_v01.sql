DECLARE
    /******************************************************************************
    NOMBRE:       43_Mig_ctgar_vehiculo_v01.sql
...
*/
   ...
   --
    CURSOR lc_mig_ctgar_vehiculo IS
     SELECT 17426 NCARGA,1 CESTMIG, a.*
     --select count(*)
      FROM mig_ctgar_vehiculo_cl1 a, mig_ctgar_det d
     WHERE 1 = 1
       AND a.mig_fk = d.mig_pk
  ORDER BY a.mig_pk
        --AND s.mig_pk = '131050100DL0200681'
        ; 
        --SELECT * FROM mig_ctgar_vehiculo_conf s WHERE s.mig_pk = '131050100DL0200681';
   --
...
BEGIN
  ...
  LOOP
    ...
    --DELETE FROM mig_ctgar_vehiculo WHERE mig_pk = l_reg_mig_mcc(i).mig_pk;
    --
    BEGIN
      --      
      FORALL i IN 1..l_reg_mig_mcc.COUNT SAVE EXCEPTIONS    
   ...
   -----------------------------------------
END;
--DELETE FROM mig_ctgar_vehiculo;
SELECT * FROM mig_ctgar_vehiculo;
SELECT * FROM mig_ctgar_vehiculo_conf s ;
SELECT * FROM mig_ctgar_vehiculo_conf s WHERE s.crespue IS NULL
...
--SELECT * FROM control_error s WHERE s.donde = '03_mig_ctgar_vehiculo' and seqerror > 9537681 order by seqerror;
DELETE FROM control_error s WHERE s.donde = '03_mig_ctgar_vehiculo'
SELECT * FROM control_error s WHERE s.donde = '37_mig_ctgar_vehiculo' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_ctgar_vehiculo);
SELECT * FROM mig_ctgar_vehiculo s, mig_personas p
