-- 08_Carga_Mig_Ctacoaseguro_v01.sql
DECLARE
   vdummy            NUMBER;
...
BEGIN
  ...
  SELECT pac_contexto.f_inicializarctx(pac_parametros.f_parempresa_t(24, 'USER_BBDD'))
    ...
    FROM dual;
    --
    -- 0. 06_Mig_ctacoaseguro.sql  (Crea registros de carga)
    -- 1. 07_Mig_ctacoaseguro.sql  (Migra registros de _CONF a MIG_)
    -- 3. - void -
    -- 4. Ejecutar este script
    -- 5. Actualizar T[s] documento DOC-108
    --
    pac_mig_axis_conf.p_migra_cargas(pid     => 'MIG_CTACOASEGURO_TXT',
                                     ptipo   => 'C',
                                     pncarga => vncarga);
    --
