DECLARE
    /******************************************************************************
    NOMBRE:       11_Mig_cuadroces_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de CUADROCES - REASEGUROS
...
*/
   ...
   --
    CURSOR lc_mig_cuadroces IS
     SELECT 17445 ncarga,1 CESTMIG,  a.MIG_PK, a.MIG_FK, a.MIG_FK2, a.NVERSIO, 0 SCONTRA, a.CTRAMO, a.CCOMREA, a.PCESION,
...
BEGIN
  ...
  LOOP
   ...
   -----------------------------------------
END;
DELETE FROM mig_cuadroces; --WHERE mig_pk IN (SELECT mig_pk FROM direccion_tmp)
SELECT * FROM mig_cuadroces WHERE cestmig = 2;  --WHERE mig_pk IN (SELECT mig_pk FROM direccion_tmp)
...
SELECT * FROM mig_logs_axis s WHERE s.ncarga = 17445 AND s.seqlog > 0;
--
SELECT s.*, c.scontra scontra1 , o.ccompani
  FROM mig_cuadroces s, mig_contratos c , mig_companias o
 WHERE s.ncarga = 17445
   AND s.cestmig = 1
   AND s.mig_fk = c.mig_pk
   AND s.mig_fk2= o.mig_pk;
--
SELECT * FROM mig_cuadroces
