-- 05_Carga_Mig_CodiContratos_v01.sql
DECLARE
   vdummy            NUMBER;
...
BEGIN
  ...
  SELECT pac_contexto.f_inicializarctx(pac_parametros.f_parempresa_t(24, 'USER_BBDD'))
    ...
    FROM dual;
    --
    -- 0. 00_Mig_Companias.sql      (Crea registros de carga)
    -- 1. 03_Mig_CodiContratos.sql  (Migra registros de _CONF a MIG_)
    -- 2. 04_Mig_Agr_contratos.sql  (Migra registros de _CONF a MIG_)
    -- 3. - void -
    -- 4. Ejecutar este script
    -- 5. Actualizar T[s] documento DOC-108
    --
    pac_mig_axis_conf.p_migra_cargas(pid     => 'MIG_COMPANIAS_TXT',
                                     ptipo   => 'C',
                                     pncarga => vncarga);
    --
