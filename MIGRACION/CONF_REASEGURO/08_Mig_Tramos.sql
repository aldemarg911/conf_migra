--
-- 08_Mig_Tramos.sql : Crea los registros de configuración
--
DECLARE
  --
   vncarga     mig_cargas.ncarga %TYPE;
   v_texto     VARCHAR2(50) := 'MIG_TRAMOS_TXT';
  -- 
BEGIN
  --
  SELECT sncarga.nextval INTO vncarga FROM dual;
     ...
     VALUES
        (vncarga, 10 /*DOC-108![mig_cargas_tab_mig]*/, 'Inserts Directos', 'MIG_TRAMOS');
    /*INSERT INTO mig_cargas_tab_mig
            (ncarga, ntab, tab_org, tab_des)     VALUES
            (17415, 9, 'Inserts Directos', 'MIG_CONTRATOS');
    INSERT INTO mig_cargas_tab_mig
            (ncarga, ntab, tab_org, tab_des)     VALUES
            (17415, 10, 'Inserts Directos', 'MIG_TRAMOS');
    INSERT INTO mig_cargas_tab_mig
            (ncarga, ntab, tab_org, tab_des)     VALUES
            (17415, 11, 'Inserts Directos', 'MIG_CUADROCES');*/
     --
...
END;
--
SELECT * FROM mig_cargas s WHERE s.ncarga = 17437 FOR UPDATE;
SELECT * FROM mig_cargas_tab_mig s WHERE s.ncarga = 17437 FOR UPDATE;
--
