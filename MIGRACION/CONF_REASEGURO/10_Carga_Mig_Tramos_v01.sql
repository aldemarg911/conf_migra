...
BEGIN
  ...
  SELECT pac_contexto.f_inicializarctx(pac_parametros.f_parempresa_t(24, 'USER_BBDD'))
    ...
    FROM dual;
    --
    -- 0. 08_Mig_Tramos.sql      (Crea registros de carga)
    -- 1. 09_Mig_Tramos.sql  (Migra registros de _CONF a MIG_)
    -- 2. - void -
    -- 3. Ejecutar este script
    -- 4. Actualizar T[s] documento DOC-108
    --
    pac_mig_axis_conf.p_migra_cargas(pid     => 'MIG_TRAMOS_TXT',
                                     ptipo   => 'C',
                                     pncarga => vncarga);
    --
