--
-- 28_Mig_PRPC_v01.sql : Crea los registros de configuración
--
DECLARE
  --
   vncarga     mig_cargas.ncarga %TYPE;
   v_texto     VARCHAR2(50) := 'PRPC_TXT'; --
  -- 
BEGIN
  --
  SELECT sncarga.nextval INTO vncarga FROM dual;
     ...
     VALUES
        (vncarga, 62 /*DOC-108![mig_cargas_tab_mig]*/, 'Inserts Directos', REPLACE(v_texto, SUBSTR(v_texto, -4)));
     --
...
END;
--
SELECT * FROM mig_cargas s WHERE s.ncarga = 17467 FOR UPDATE;
SELECT * FROM mig_cargas_tab_mig s WHERE s.ncarga = 17467 FOR UPDATE;
