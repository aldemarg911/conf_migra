DECLARE
    /******************************************************************************
    NOMBRE:       08_Mig_Detrecibos_v01.sql
    PROPÓSITO:    Procedimiento que ejecuta carga de mig_detrecibos - PRODUCCION
...
*/
   ...
   --
    CURSOR lc_mig_detrecibos IS
     ...
     --select count(*)
      FROM mig_detrecibos_cl2 a, mig_recibos s
     WHERE 1 = 1
        AND a.mig_fk  = s.mig_pk
        AND s.nrecibo <> 0
      ORDER BY a.mig_pk
        ;
        --SELECT * FROM mig_detrecibos_conf s;
        --SELECT * FROM mig_detrecibos s for update WHERE s.mig_pk = 'RD30094778';
...
BEGIN
  ...
  LOOP
   ...
   -----------------------------------------
END;
--DELETE FROM mig_detrecibos;
/*SELECT COUNT(*) FROM mig_detrecibos;
SELECT COUNT(*) FROM mig_detrecibos s WHERE ncarga = 17435 AND cestmig != 1;
SELECT * FROM mig_detrecibos s WHERE s.crespue IS NULL
...
DELETE FROM control_error s WHERE s.donde = '08_mig_Detrecibos'
SELECT * FROM control_error s WHERE s.donde = '08_mig_Detrecibos' order by seqerror;
SELECT * FROM mig_personas WHERE mig_pk IN (SELECT DISTINCT mig_fk FROM mig_detrecibos);
SELECT * FROM mig_detrecibos s, mig_personas p
WHERE s.mig_fk = p.mig_pk
  AND p.idperson <> 0;
--
SELECT * FROM mig_detrecibos s;
